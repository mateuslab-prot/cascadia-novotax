...
def convert_mods_to_shifts(db, pep):
    ...
    ii = 0
    while ii < len(toks):
        converted_toks.append(toks[ii])
        if toks[ii] == '[':
            try:
                mod_mass = db.by_name(toks[ii+1]).monoisotopic_mass
                if mod_mass < 0:
                    mod_mass = str(round(mod_mass,2))
                else:
                     mod_mass = '+' + str(round(mod_mass,2))
                converted_toks.append(mod_mass)
                ii += 1
            except(KeyError):
                pass
        ii += 1
    return(''.join(converted_toks))
